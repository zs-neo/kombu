...
class test_Channel:
    ...
    def test_qos_restore_visible(self):
        ...
        qos = rediscluster.QoS(self.channel)
        ...
        qos._vrestore_count = 0
        client.setnx.side_effect = rediscluster.MutexHeld()
        qos.restore_visible()
    ...
    def test_brpop_read_gives_None(self):
        ...
        with pytest.raises(rediscluster.Empty):
    ...
    def test_connparams_allows_slash_in_db(self):
        ...
        assert 'db' not in self.channel._connparams()
    ...
    def test_new_queue_with_auto_delete(self):
        rediscluster.Channel._new_queue(self.channel, 'george', auto_delete=False)
        assert 'george' not in self.channel.auto_delete_queues
        rediscluster.Channel._new_queue(self.channel, 'elaine', auto_delete=True)
    ...
    def test_get_prefixed_client(self):
        ...
        with pytest.raises(KeyPrefixNotSupport):
            rediscluster.Channel._get_client(self.channel)

    def test_get_response_error(self):
        from redis.exceptions import ResponseError
        assert rediscluster.Channel._get_response_error(self.channel) is ResponseError
    ...
    def test_register_with_event_loop(self):
        ...
        loop = Mock(name='loop')
        rediscluster.Transport.register_with_event_loop(transport, conn, loop)
    ...
    def test_transport_on_readable(self):
        ...
        rediscluster.Transport.on_readable(transport, 13)
        cycle.on_readable.assert_called_with(13)

    def test_transport_connection_errors(self):
        """Ensure connection_errors are populated."""
        assert rediscluster.Transport.connection_errors

    def test_transport_channel_errors(self):
        """Ensure connection_errors are populated."""
        assert rediscluster.Transport.channel_errors

    def test_transport_driver_version(self):
        assert rediscluster.Transport.driver_version(self.connection.transport)
...
class test_MultiChannelPoller:
    ...
    def test_handle_event(self):
        ...
        p.handle_event(13, rediscluster.READ)
        ...
        p.handle_event(13, rediscluster.ERR)
        chan._poll_error.assert_called_with('BRPOP')

        p.handle_event(13, ~(rediscluster.READ | rediscluster.ERR))
